#!/usr/bin/env python3
"""
train_yolo11l.py — Fine-tune YOLO11l on secure_cv_v4 (same recipe as train_yolo11x.py).

Usage:
    cd object-detection-main
    uv run scripts/train_yolo11l.py

Output:
    - Best weights copied to models/yolo/secure_cv_best_l.pt
    - Log written to training_log_11l.txt
"""
...
FINAL_WEIGHTS = REPO_ROOT / "models" / "yolo" / "secure_cv_best_l.pt"
LOG_FILE = REPO_ROOT / "training_log_11l.txt"

# RTX 2080 Ti (11 GB) — 11l @ 960 batch 4 (same as 11x recipe; L uses less VRAM)
BASE_MODEL = "yolo11l.pt"
EPOCHS = 150
IMGSZ = 960
BATCH_SIZE = 4
PATIENCE = 20
PROJECT_NAME = "secure_cv_training_11l"
...
def main():
    log_lines: list[str] = []
    log("=" * 72, log_lines)
    log("  SECURE CV — YOLO11l TRAINING (same recipe as 11x)", log_lines)
    log("=" * 72, log_lines)
    ...
    if FINAL_WEIGHTS.exists():
        backup = FINAL_WEIGHTS.with_name(
            f"secure_cv_best_l_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pt"
        )
    ...
    log(
        "Next: uv run scripts/export_engine.py --weights models/yolo/secure_cv_best_l.pt --imgsz 960",
        log_lines,
    )
    log("Then set MODEL_SELECT=9 in .env and restart the server.", log_lines)
